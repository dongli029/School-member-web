...
const studentSchema = new Schema(
  {
    ...
    major: {
      ...
      required: [
        ...
        // },
        "merit大於3000的學生major為必填項目", //客制化報錯提示
      ],
      enum: [
        "Chemistry",
        "CS",
    ...
    scholarship: {
      merit: {
        type: Number,
        min: 0,
        max: [5000, "merit for student is too much"],
        default: 0,
      },
  ...
  },
  {
    ...
    methods: {
      printTotalScholarship() {
        ...
        return this.scholarship.merit + this.scholarship.other + " dollors.";
      },
    },
    statics: {
      async findAllMajorStudents(course) {
        //console.log(this);
        //這裡的this是代表用此schema創建的model
        //創一個變數data用來接收 用await 等待promise物件fullfilled後回傳的參數(這邊是一個array)
        data = await this.find({ major: course }).exec();
        try {
